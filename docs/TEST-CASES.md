...
| ID | 场景 | 前置条件 | 测试步骤 | 期望结果 | 优先级 |
|----|------|----------|----------|----------|--------|
| TC-007 | 置忙成功 | 已登录，状态为空闲 | 1. 点击"置忙"按钮 | setAgentStatus({ status: 2 }) 成功，状态变为"忙碌" | P0 |
| TC-008 | 置忙失败-非空闲状态 | 状态为忙碌/通话中 | 1. 检查"置忙"按钮状态 | 按钮禁用 | P1 |
| TC-009 | 置闲成功 | 已登录，状态为忙碌/整理 | 1. 点击"置闲"按钮 | setAgentStatus({ status: 1 }) 成功，状态变为"空闲" | P0 |
| TC-010 | 置闲失败-非忙碌状态 | 状态为空闲/通话中 | 1. 检查"置闲"按钮状态 | 按钮禁用 | P1 |
| TC-011 | 置忙置闲按钮初始状态 | 未登录 | 1. 检查按钮状态 | 置忙/置闲按钮均禁用 | P1 |
...
| ID | 场景 | 前置条件 | 测试步骤 | 期望结果 | 优先级 |
|----|------|----------|----------|----------|--------|
| TC-026 | 开始咨询成功 | 已登录，通话中 | 1. 点击"开始咨询"按钮 2. 输入目标号码 | startAtxfer() 成功，状态变为咨询中，Toast 提示"咨询已发起" | P0 |
| TC-027 | 开始咨询失败-非通话状态 | 状态为空闲 | 1. 检查"开始咨询"按钮状态 | 按钮禁用 | P1 |
| TC-028 | 取消咨询成功 | 已登录，咨询中 | 1. 点击"取消咨询"按钮 | cancelAtxfer() 成功，状态恢复，Toast 提示"咨询已取消" | P0 |
| TC-029 | 取消咨询失败-非咨询状态 | 状态为空闲 | 1. 检查"取消咨询"按钮状态 | 按钮禁用 | P1 |
| TC-030 | 恢复咨询成功 | 已登录，咨询中 | 1. 点击"恢复咨询"按钮 | resumeAtxfer() 成功，Toast 提示"咨询已恢复" | P0 |
| TC-031 | 恢复咨询失败-非咨询状态 | 状态为空闲 | 1. 检查"恢复咨询"按钮状态 | 按钮禁用 | P1 |
| TC-032 | 完成转接成功 | 已登录，咨询中 | 1. 点击"完成转接"按钮 | completeAtxfer() 成功，转接完成，Toast 提示"转接已完成" | P0 |
| TC-033 | 完成转接失败-非咨询状态 | 状态为空闲 | 1. 检查"完成转接"按钮状态 | 按钮禁用 | P1 |
| TC-034 | 三方咨询成功 | 已登录，咨询中 | 1. 点击"三方咨询"按钮 | threewayAtxfer() 成功，建立三方通话，Toast 提示"三方咨询已建立" | P1 |
| TC-035 | 三方咨询失败-非咨询状态 | 状态为空闲 | 1. 检查"三方咨询"按钮状态 | 按钮禁用 | P1 |
| TC-036 | 盲转成功 | 已登录，通话中 | 1. 点击"盲转"按钮 2. 输入目标号码 | blxfer() 成功，Toast 提示"盲转已完成" | P0 |
| TC-037 | 盲转失败-非通话状态 | 状态为空闲 | 1. 检查"盲转"按钮状态 | 按钮禁用 | P1 |
| TC-038 | 转接按钮初始状态 | 未登录 | 1. 检查所有转接按钮状态 | 开始咨询/取消咨询/恢复咨询/完成转接/三方咨询/盲转 按钮均禁用 | P1 |
