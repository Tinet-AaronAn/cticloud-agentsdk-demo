...
| ID | 场景 | 前置条件 | 测试步骤 | 期望结果 | 优先级 |
|----|------|----------|----------|----------|--------|
| TC-026 | 保持成功 | 已登录，通话中 | 1. 点击"保持"按钮 | hold() 成功，状态变为保持，Toast 提示"已保持" | P0 |
| TC-027 | 保持失败-非通话状态 | 状态为空闲 | 1. 检查"保持"按钮状态 | 按钮禁用 | P1 |
| TC-028 | 恢复成功 | 已登录，保持状态 | 1. 点击"恢复"按钮 | unhold() 成功，恢复通话，Toast 提示"已恢复" | P0 |
| TC-029 | 恢复失败-非保持状态 | 状态为空闲 | 1. 检查"恢复"按钮状态 | 按钮禁用 | P1 |
| TC-030 | 静音成功 | 已登录，通话中 | 1. 点击"静音"按钮 | mute() 成功，静音生效，Toast 提示"已静音" | P0 |
| TC-031 | 静音失败-非通话状态 | 状态为空闲 | 1. 检查"静音"按钮状态 | 按钮禁用 | P1 |
| TC-032 | 取消静音成功 | 已登录，静音状态 | 1. 点击"取消静音"按钮 | unmute() 成功，恢复声音，Toast 提示"已取消静音" | P0 |
| TC-033 | 取消静音失败-非静音状态 | 状态为空闲 | 1. 检查"取消静音"按钮状态 | 按钮禁用 | P1 |
| TC-034 | 发送 DTMF 成功 | 已登录，通话中 | 1. 点击"发送DTMF"按钮 2. 输入数字 | sendDtmf() 成功，Toast 提示"DTMF 已发送" | P1 |
| TC-035 | 发送 DTMF 失败-非通话状态 | 状态为空闲 | 1. 检查"发送DTMF"按钮状态 | 按钮禁用 | P1 |
| TC-036 | 通话控制按钮初始状态 | 未登录 | 1. 检查所有通话控制按钮状态 | 保持/恢复/静音/取消静音/发送DTMF 按钮均禁用 | P1 |
